    ...
    <div className="team-info">
      <img
        ...
        onError={(e) => {
          ...
          e.currentTarget.src = "/img/no-logo.png";
        }}
        alt=""
        className="team-logo"
      />
  ...
  return (
    <div className="compact-scoreboard sport-setbased">
      ...
      <div className="scoreboard-header volleyball-header">
        {displayData.tournamentName && (
          <span className="tournament-name">{displayData.tournamentName}</span>
        )}
        <span className="match-type">{displayData.matchType}</span>
        {displayData.court && (
          <span className="court">{displayData.court}</span>
        )}
      </div>
      ...
      <div className="scoreboard-content gap-8">
        {/* Team A */}
        <div
          className={`team-row volleyball-row ${displayData.serviceTeam === "A" ? "serving" : ""}`}
        >
        ...
        {/* Team B */}
        <div
          className={`team-row volleyball-row ${displayData.serviceTeam === "B" ? "serving" : ""}`}
        >
          {/* Service zone - only show image when this team has service */}
          <div className="service-zone">
    ...
    <div className={`compact-scoreboard sport-${sport}`}>
      ...
      <div className="scoreboard-content">
        ...
        <div className="team-row">
          ...
          <div className="score-section">
            <div className="cards-display">
              {yellowCards1 > 0 && (
                <span className="card yellow">🟨 {yellowCards1}</span>
              )}
              {redCards1 > 0 && (
                <span className="card red">🟥 {redCards1}</span>
              )}
            </div>
            <span className="score">{displayData.teamA.score}</span>
          </div>
        ...
        <div className="divider football-divider">
          {displayData.chrono && (
            <span className="chrono">{displayData.chrono}</span>
          )}
        </div>
        ...
        <div className="team-row">
          ...
          <div className="score-section">
            <div className="cards-display">
              {yellowCards2 > 0 && (
                <span className="card yellow">🟨 {yellowCards2}</span>
              )}
              {redCards2 > 0 && (
                <span className="card red">🟥 {redCards2}</span>
              )}
            </div>
            <span className="score">{displayData.teamB.score}</span>
          </div>
...
function BasketballScoreboard({
  ...
  const [prevSseShotClock, setPrevSseShotClock] = useState<string | undefined>(undefined);
  const [isRunning, setIsRunning] = useState(false);
  const [prevSseRunning, setPrevSseRunning] = useState<boolean | undefined>(undefined);

  // State adjustment: sync shot clock from SSE
  if (sseShotClock && prevSseShotClock !== sseShotClock) {
    const newTenths = Math.round(parseFloat(sseShotClock) * 10);
    const prevTenths = prevSseShotClock
      ? Math.round(parseFloat(prevSseShotClock) * 10)
      : null;
    setPrevSseShotClock(sseShotClock);
    ...
    if (prevTenths === null) {
      ...
      // Value decreased - check drift using current state
      const drift = Math.abs(newTenths - (localShotClock ?? 0));
      if (drift > 15) {
        setLocalShotClock(newTenths);
      }
    }
  ...
  // State adjustment: sync running state from SSE
  if (sseChronoRunning !== undefined && prevSseRunning !== sseChronoRunning) {
    setPrevSseRunning(sseChronoRunning);
    setIsRunning(sseChronoRunning);
  }

  // State adjustment: stop countdown when clock hits 0
  if (localShotClock === 0 && isRunning) {
    setIsRunning(false);
  }

  // Countdown interval (setState only inside callback, not synchronously in effect body)
  useEffect(() => {
    if (!isRunning) return;
  ...
  // Format for display
  const displayShotClock =
    localShotClock !== null ? (localShotClock / 10).toFixed(1) : sseShotClock;
    ...
    <div className="compact-scoreboard sport-basketball">
      ...
      <div className="scoreboard-content">
        ...
        <div className="timer-section justify-center gap-6">
          {displayData.chrono && (
            <span className="chrono main-timer">{displayData.chrono}</span>
          )}
          {displayShotClock && (
            <span className="chrono shot-clock">{displayShotClock}</span>
          )}
        </div>
      </div>
    </div>
...
function PetanqueScoreboard({
  ...
  console.log(
    "%c[PetanqueScoreboard] 🎱 Rendering",
  ...
  return (
    <div className="compact-scoreboard sport-petanque">
      ...
      <div className="scoreboard-content petanque-content">
        {/* Team A row */}
        <div
          className={`team-row petanque-row ${cochonnetTeam === "A" ? "has-cochonnet" : ""}`}
        >
        ...
        {/* Team B row */}
        <div
          className={`team-row petanque-row ${cochonnetTeam === "B" ? "has-cochonnet" : ""}`}
        >
          {/* Cochonnet zone - only show image+label when this team has cochonnet */}
          <div className="cochonnet-zone">
            {cochonnetTeam === "B" && (
      ...
      {/* Winner banner */}
      {winner && (
        <div className="winner-banner">🏆 Vainqueur : {winner} 🏆</div>
      )}
    </div>
  );
}
        ...
        {currentThrows.length > 0 && (
          <div className="throws-display">
            {currentThrows.map((t, i) => (
              <span key={i} className="throw">
                {t}
              </span>
            ))}
            <span className="throw-total">
...
function useDisplayData(match: Match, liveData: LiveData | null) {
  return useMemo(() => {
    ...
    const teamBLogo =
      liveData?.logo2 && liveData.logo2.length > 0
        ? liveData.logo2
...
export default function CompactScoreboard({
  ...
  console.log(
    `[CompactScoreboard] Sport: "${sport}", Match: ${match?.id}, HasLiveData: ${!!liveData}`,
  );
  ...
  switch (normalizedSport) {
    ...
    case "basketball":
      return (
        <BasketballScoreboard displayData={displayData} liveData={liveData} />
      );
    case "petanque":
      return (
        <PetanqueScoreboard displayData={displayData} liveData={liveData} />
      );
    case "flechettes":
      return (
        <FlechettesScoreboard displayData={displayData} liveData={liveData} />
      );
    default:
      // Generic fallback
      return (
