...
export function useVolleyballMatch(initialMatchId: string | null) {
    ...
    useEffect(() => {
        ...
        async function fetchMatchData() {
            try {
                ...
                console.log('[Volleyball Hook] Match data updated with tournamentId:', tournamentId, 'numericId:', match.id);
    ...
    function addPoint(team: "A" | "B") {
        setMatchData(prev => {
            ...
            if (matchFinished) {
                // En mode correction : ajout de point sans logique de set
            ...
            // Vérifier si le set est fini (toujours à 25 points)
    ...
    const subPoint = (team: "A" | "B") =>
        setMatchData((p: MatchDataWithTournament) => {
            ...
            // Si le score actuel est un score de fin de set (ex: 23-25),
    ...
    const submitMatchResult = async () => {
        ...
        const result = await submitMatchResultWithPropagation({
            ...
            onError: (error) => {
                console.error('[Volleyball Hook] ❌ Error:', error);
                alert('Erreur lors de la fin du match : ' + error);
            },
        });

        console.log('[Volleyball Hook] Submit result:', result);
    };
