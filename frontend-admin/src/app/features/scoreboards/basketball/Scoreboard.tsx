...
function PlayerSelectModal({ title, teamName, players, selectedId, onSelect, onCancel, onConfirm }: PlayerSelectModalProps) {
  return (
    <div className="goal-modal-overlay">
      <div className="goal-modal-card" role="dialog" aria-modal="true">
...
export default function BasketballTableMarquagePage() {
  ...
  return (
    <main className="basketball-root">
      ...
      {/* Modale sélection joueur */}
      {pendingGoalTeam && (
        <PlayerSelectModal
          title={`🏀 Qui a marqué ? (+${pendingScorePoints} pt${(pendingScorePoints ?? 0) > 1 ? "s" : ""})`}
          ...
          onCancel={() => { cancelScoreModal(); setSelectedScorerPlayerId(null); }}
          onConfirm={() => {
            if (selectedScorerPlayerId === null || !pendingGoalTeam) return;
            confirmScore(pendingGoalTeam, selectedScorerPlayerId === "none" ? undefined : (selectedScorerPlayerId as number));
            setSelectedScorerPlayerId(null);
          }}
        />
      )}

      {/* FAB panneau d'événements */}
        ...
        <div className="goal-events-fab">
          <div className="goal-events-shell">
            <button
              ...
              aria-label="Afficher les paniers"
            >
            ...
            {showEventPanel && (
              <div className="goal-events-panel">
                <p className="goal-events-panel-title">Paniers enregistrés</p>
                      ...
                      <article key={e.localId} className="goal-event-item">
                        <div className="goal-event-main">
                          <div className="goal-event-playerline">
                            ...
                            <span className="font-bold text-xs ml-1">+{e.points} pt{e.points > 1 ? "s" : ""}</span>
                          </div>
