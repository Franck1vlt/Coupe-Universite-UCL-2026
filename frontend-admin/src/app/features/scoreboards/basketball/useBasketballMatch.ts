...
export function useBasketballMatch(initialMatchId: string | null) {
  ...
  useEffect(() => {
    ...
    async function fetchMatchData() {
      try {
        console.log(
          "[Basketball Hook] 📝 Fetching match data for matchId:",
          initialMatchId,
        );
        ...
        if (match.phase_id) {
          try {
            console.log(
              "[Basketball Hook] 📝 Fetching tournament ID via phase_id:",
              match.phase_id,
            );
            const phaseResponse = await fetch(
              `${process.env.NEXT_PUBLIC_API_URL}/tournament-phases/${match.phase_id}`,
            );
            if (phaseResponse.ok) {
              const phaseData = await phaseResponse.json();
              tournamentId = phaseData.data.tournament_id;
              console.log(
                "[Basketball Hook] ✅ Tournament ID found:",
                tournamentId,
              );
            }
          } catch (err) {
            console.error(
              "[Basketball Hook] ❌ Erreur récupération tournamentId:",
              err,
            );
          }
        }
        ...
        if (match.team_sport_a_id) {
          console.log(
            "[Basketball Hook] 📝 Fetching team A, team_sport_id:",
        ...
        if (match.team_sport_b_id) {
          console.log(
            "[Basketball Hook] 📝 Fetching team B, team_sport_id:",
        ...
        if (scheduleResponse.ok) {
          ...
          if (scheduleData.data?.court_id) {
            const courtResponse = await fetch(
              `${process.env.NEXT_PUBLIC_API_URL}/courts/${scheduleData.data.court_id}`,
            );
            if (courtResponse.ok) {
  ...
  // Possession de balle (null = non défini, invisible)
  const [possession, setPossession] = useState<"A" | "B" | null>(null);
  const togglePossession = () =>
    setPossession((prev) => (prev === "A" ? "B" : "A"));
  ...
  // Démarre uniquement le chrono principal
  const startMainChrono = () => {
    ...
    intervalRef.current = window.setInterval(() => {
      setMatchData((p) => {
        ...
        if (newTime === 0 && p.chrono.time > 0) {
          ...
          return { ...p, chrono: { ...p.chrono, time: 0, running: false } };
        }

        return { ...p, chrono: { ...p.chrono, time: newTime } };
      });
    }, 1000);

    setMatchData((prev) => ({ ...prev, chrono: { ...prev.chrono, running: true } }));
  };

  // Arrête le chrono principal + le shot clock (règle officielle basketball)
  const stopMainChrono = () => {
    if (intervalRef.current !== null) {
      window.clearInterval(intervalRef.current);
      intervalRef.current = null;
    }
    ...
    setMatchData((prev) => ({ ...prev, chrono: { ...prev.chrono, running: false } }));
  };

  // Démarre uniquement le shot clock
  const startShotClock = () => {
    if (shotClockIntervalRef.current !== null) return;
    ...
    shotClockIntervalRef.current = window.setInterval(() => {
      if (shotClockRunningRef.current) {
        setShotClockState((prev) => {
          ...
          if (next === 0) {
            // Shot clock à 0 : buzzer + arrêt des deux chronos + reset à 24s
            buzzer.play();
            if (intervalRef.current !== null) {
              window.clearInterval(intervalRef.current);
              intervalRef.current = null;
            ...
            setMatchData((p) => ({ ...p, chrono: { ...p.chrono, running: false } }));
  ...
  // Arrête uniquement le shot clock
  const stopShotClock = () => {
    shotClockRunningRef.current = false;
    if (shotClockIntervalRef.current !== null) {
      window.clearInterval(shotClockIntervalRef.current);
  ...
  // Arrête les deux chronos (utilisé pour buzzer-stop, fin de match, etc.)
  const stopChrono = () => {
    stopMainChrono();
    stopShotClock();
  };

  // Alias pour démarrer les deux (compatibilité avec handleStart)
  const startChrono = () => {
    startMainChrono();
    startShotClock();
  };
  ...
  /** ---------- HELPERS ---------- */
  const teamKey = (t: "A" | "B"): "teamA" | "teamB" =>
    t === "A" ? "teamA" : "teamB";
  ...
  const addFault = (team: "A" | "B", amount: number) =>
    setMatchData((p) => {
      const k = teamKey(team);
      return { ...p, [k]: { ...p[k], fouls: p[k].fouls + amount } };
    });

  const subFault = (team: "A" | "B", amount: number) =>
    setMatchData((p) => {
      const k = teamKey(team);
      return { ...p, [k]: { ...p[k], fouls: Math.max(0, p[k].fouls - amount) } };
    });
  ...
  const togglePeriod = () => {
    ...
    setMatchData((p) => ({
      ...p,
      teamA: { ...p.teamA, fouls: 0 },
      teamB: { ...p.teamB, fouls: 0 },
      chrono: { ...p.chrono, time: HALF_TIME_DURATION, running: false },
    }));
    // Arrêter les chronos
    if (intervalRef.current !== null) {
  ...
  const handleEnd = async () => {
    ...
    if (snapshot.length > 0 && initialMatchId) {
      ...
      try {
        await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/matches/${matchNumId}/events/batch`,
          {
            method: "POST",
            headers: {
  ...
  const displayShotClock =
    shotClock > gameTimeInTenths ? gameTimeInTenths : shotClock;
  ...
  useEffect(() => {
    ...
    console.log(
      "[Basketball Sync] shotClock state:",
      shotClock,
      "| formatted:",
      formattedShotClock,
      "| chrono:",
    ...
    try {
      const payload = {
        ...
        fouls1: matchData.teamA.fouls,
        fouls2: matchData.teamB.fouls,
        possession,
      ...
      if (initialMatchId) {
        localStorage.setItem(
          `liveBasketballMatch_${initialMatchId}`,
  ...
  const swapSides = () => {
    ...
    setPossession((prev) => (prev === "A" ? "B" : prev === "B" ? "A" : null));
  };
