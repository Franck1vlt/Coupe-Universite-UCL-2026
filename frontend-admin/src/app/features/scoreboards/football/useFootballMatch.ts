...
export function useFootballMatch(initialMatchId: string | null) {
  ...
  const [pendingGoalTeam, setPendingGoalTeam] = useState<"A" | "B" | null>(
    null,
  );
    ...
    async function fetchMatchData() {
      ...
      try {
        const statusResponse = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/matches/${initialMatchId}/status`,
        );
        if (statusResponse.ok) {
          const statusData = await statusResponse.json();
          console.log(
            "[Football Hook] Current match status:",
            statusData.data?.status,
          );
        }
      } catch (e) {
        console.error(
          "[Football Hook] Erreur lors de la récupération du statut:",
          e,
        );
      }
      try {
        console.log(
          "[Football Hook] Fetching match data for matchId:",
          initialMatchId,
        );
        ...
        if (match.team_sport_a_id) {
          console.log(
            "[Football Hook] Fetching team_sport_a:",
          ...
          if (teamSportAResponse.ok) {
            ...
            console.log(
              "[Football Hook] TeamSport A data:",
              teamSportAData.data,
            );
        ...
        if (match.team_sport_b_id) {
          console.log(
            "[Football Hook] Fetching team_sport_b:",
          ...
          if (teamSportBResponse.ok) {
            ...
            console.log(
              "[Football Hook] TeamSport B data:",
              teamSportBData.data,
            );
        ...
        console.log(
          "[Football Hook] Fetching schedule for match:",
          initialMatchId,
        );
        ...
        console.log(
          "[Football Hook] Schedule response status:",
        ...
        console.log(
          "[Football Hook] Final values - TeamA:",
          teamAName,
          "TeamB:",
          teamBName,
          "MatchType:",
    ...
    try {
      ...
      if (initialMatchId) {
        localStorage.setItem(
          `liveFootballMatch_${initialMatchId}`,
    ...
    if (currentPendingEvents.length > 0) {
      ...
      if (matchId) {
        try {
          ...
          console.error(
            "[Football Hook] Erreur sauvegarde événements batch:",
            e,
          );
        }
      }
