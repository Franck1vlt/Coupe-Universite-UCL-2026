...
export function useBadmintonMatch(initialMatchId: string | null) {
    ...
    /** Nouveau addPoint : ouvre la modale si des joueurs sont enregistrés, sinon direct */
    ...
    /** Confirme le point depuis la modale */
    ...
    useEffect(() => {
        try {
            ...
            if (initialMatchId) {
                localStorage.setItem(`liveBadmintonMatch_${initialMatchId}`, JSON.stringify(payload));
            }
